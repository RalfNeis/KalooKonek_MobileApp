...
export default function AnnouncementDetails() {
  // ADDED: Catching the 'time' variable
  const { title, date, time, body } = useLocalSearchParams();
    ...
    <ScrollView className="flex-1 bg-white">
      ...
      <View className="bg-red-50 p-6 pt-10 border-b border-red-100">
        ...
        <View className="flex-row items-center gap-2 mb-2">
          ...
          {/* ADDED: Displaying the time conditionally next to the date */}
          <Text className="text-sm text-gray-600">{date || 'Recent'}{time ? ` • ${time}` : ''}</Text>
        </View>
